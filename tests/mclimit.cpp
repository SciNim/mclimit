				       ...
				       TRandom * generator)
{
	...
	for (Int_t channel = 0;
		...
		if ((Double_t) ((TH1 *) (fluctuated->GetSignal()->At(channel)))->GetBinContent(bin) != 0) {
		    ...
		    //if(rate < 0) std::cout << "IT IS TOO " << rate << std::endl;
		       ...
		       bool init, TRandom * generator, bool stat)
{
	...
	for (Int_t channel = 0; channel <= input->GetSignal()->GetLast(); channel++) {
		...
		for(int i=1; i<=newsignal->GetNbinsX(); i++) {
		    Double_t g = generator->Gaus(0,oldsignal->GetBinError(i));

		    newsignal->SetBinContent(i,oldsignal->GetBinContent(i) + g);
		    //if (g < 0) {
		    //	std::cout << " is " << g << " and " << newsignal->GetBinContent(i)
		    //		  << " before was " << oldsignal->GetBinContent(i) << std::endl;
		    //}
